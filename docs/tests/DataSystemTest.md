...
Once the game is running, open your **Command Bar** (`View -> Command Bar`) and paste the snippets below. 
...
**Command:**
```lua
require(game.ServerScriptService.serverNormalIsland.Tests.DataSystemTest).GiveTestItem("Player1", "TestSword", 1)
```
*(If you are the only one in the game, you can just omit the arguments like so `require(game.ServerScriptService.serverNormalIsland.Tests.DataSystemTest).GiveTestItem()` to default to yourself and the `"TestSword"`)*
...
**Command:**
```lua
require(game.ServerScriptService.serverNormalIsland.Tests.DataSystemTest).CheckData("Player1")
```
...
**Command:**
```lua
require(game.ServerScriptService.serverNormalIsland.Tests.DataSystemTest).ForceSave("Player1")
```
